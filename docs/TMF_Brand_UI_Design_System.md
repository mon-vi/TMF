...
**Status:** Inferred & Documented from Existing Implementation
...
### Font Families
*   **Heading Font (Serif):** Estimated as *Playfair Display*, *Merriweather*, or *LORA*. (High Confidence of Serif style).
*   **Body Font (Sans-Serif):** Estimated as *Inter*, *Roboto*, or *System UI*. (High Confidence of Sans-serif style).
...
| **Subheading** | Sans-Serif | 14px | Bold | 1.5 | Section kickers ("IMPACT", "INVEST IN THE FUTURE") - *Always Uppercase* |
...
| **Border Radii** | Global | Images are rounded (`~24px`), Icons are rounded (`~8px`), Buttons/Forms are sharp (`0px`). | Low | *None.* This appears to be an intentional, stylistic choice (Principle #2) rather than an error. |
...
:root {
  ...
  /* Shadows */
  --shadow-overlay: 0 4px 6px -1px rgba(0, 0, 0, 0.1), 0 2px 4px -1px rgba(0, 0, 0, 0.06);
}
```
